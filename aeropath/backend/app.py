...
from firebase_auth import verify_firebase_token, set_user_role, require_auth
...
def set_role():
    """Set a role (user/ambulance) on a Firebase user via custom claims."""
    data = request.get_json()
    id_token = data.get("idToken")
    ...
    try:
        user_info = verify_firebase_token(id_token)
        success = set_user_role(user_info["uid"], role)
        if success:
            return jsonify({"status": "success", "message": f"Role '{role}' assigned"})
        return jsonify({"status": "error", "message": "Could not set role"}), 500
    except ValueError as e:
        return jsonify({"status": "error", "message": str(e)}), 401
